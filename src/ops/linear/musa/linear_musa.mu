...
void launchLinear(std::byte *out,
    ...
    auto &resource = llaisys::device::musa::getResource( device_id);

    const mublasHandle_t handle = resource.mublasHandle();

    checkMublas(
        mublasSetStream(handle, reinterpret_cast<musaStream_t>(stream)),
        "mublasSetStream");
    ...
    const half alpha_f16 = __float2half(1.0f);
    const half beta_f16 = __float2half(0.0f);

    const bool use_fp16_compute = data_type == MUSA_R_16F;

    const void *alpha = use_fp16_compute
        ? static_cast<const void *>(&alpha_f16)
        : static_cast<const void *>(&alpha_f32);

    const void *beta = use_fp16_compute
        ? static_cast<const void *>(&beta_f16)
        : static_cast<const void *>(&beta_f32);

    const mublasComputeType_t compute_type = use_fp16_compute
        ? MUBLAS_COMPUTE_16F
        : data_type == MUSA_R_32F
            ? MUBLAS_COMPUTE_32F_FAST_TF32
            : MUBLAS_COMPUTE_32F;

    const mublasGemmAlgo_t algorithm = data_type == MUSA_R_32F ? MUBLAS_GEMM_DEFAULT : MUBLAS_GEMM_DEFAULT_TENSOR_OP;
    ...
    if (bias != nullptr) {
        ...
        addBiasKernel<T><<<grid_size, BLOCK_SIZE, 0, reinterpret_cast<musaStream_t>(stream)>>>(
        ...
        if (status != musaSuccess) {
            throw std::runtime_error(
                std::string("MUSA Linear bias kernel failed: ")
...
void linear(std::byte *out,
    ...
    case LLAISYS_DTYPE_BF16:
        return launchLinear<__mt_bfloat16>(out,
            in,
            weight,
            bias,
            MUSA_R_16BF,
