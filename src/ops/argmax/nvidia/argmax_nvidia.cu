...
void launchArgmax(std::byte *max_idx,
    ...
    argmaxKernel<T><<<1, BLOCK_SIZE, 0, reinterpret_cast<cudaStream_t>(stream)>>>(
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(
            std::string("CUDA Argmax kernel launch failed: ")
...
void argmax(std::byte *max_idx,
    ...
    case LLAISYS_DTYPE_BF16:
        return launchArgmax<__nv_bfloat16>(max_idx,
            max_val,
            vals,
            numel,
            stream);
